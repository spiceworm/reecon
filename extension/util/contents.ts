...
export const execute = async () => {
  ...
  if (await storage.shouldExecuteContentScript()) {
    ...
    const threads = await backgroundMessage.processThreads(
      producerSettings,
      urlPaths
    )
    ...
    const ignoredUsernames = new Set(
      ignoredRedditors.map((obj) => obj.username)
    )
    const redditors = await backgroundMessage.processRedditors(
      producerSettings,
      usernames,
      ignoredUsernames
    )
    await dom.annotateUsernames(
      redditors,
      ignoredRedditors,
      usernameElementsMap,
      contentFilter

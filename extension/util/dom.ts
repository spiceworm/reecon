...
export const annotateThreads = async (
  ...
  for (const thread of threads) {
    ...
    if (threadDataSpan === null) {
      ...
      let threadTitleElement = threadRow.querySelector(
...
export const annotateUsernames = async (
  redditors: types.Redditor[],
  ...
  for (let redditor of redditors) {
    ...
    if (username in usernameElementsMap) {
      ...
      for (let existingDataSpan of document.getElementsByName(
        redditorDataSpanName
      )) {
      ...
      for (let linkElement of usernameElementsMap[username]) {
        linkElement.parentElement.insertAdjacentElement(
          "beforeend",
          redditorDataSpan
        )
      }
...
const getThreadElements = () => {
  return document.querySelectorAll(
    "[data-permalink]:not(.comment, .ad-container)"
  )
}
...
export const getUsernameElementsMap = () => {
  ...
  for (const el of getUsernameElements() as HTMLCollectionOf<HTMLLinkElement>) {
    ...
    username in usernameElements
      ? usernameElements[username].push(el)
      : (usernameElements[username] = [el])
  }

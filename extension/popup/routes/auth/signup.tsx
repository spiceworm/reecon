...
export const Signup = () => {
  ...
  const { error: loginError, isLoading: loginIsLoading } = useSWRImmutable(
    signupResponse && !signupError && !signupIsLoading
      ? ["/api/v1/auth/token/", signupCredentials]
      : null,
    ([urlPath, credentials]) => api.post(urlPath, credentials),
    {
    ...
    <base.Unauthenticated>
      ...
      {!signupError ? null : (
        <UncontrolledAlert color={"danger"}>
          {JSON.parse(signupError.message).detail}
        </UncontrolledAlert>
      )}
      {!loginError ? null : (
        <UncontrolledAlert color={"danger"}>
          {JSON.parse(loginError.message).detail}
        </UncontrolledAlert>
      )}
